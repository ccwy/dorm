...
:: ===== 构建Asset Agent（可选步骤，已有二进制时跳过）=====
echo.
echo [%date% %time:~0,8%] 检查Asset Agent二进制...
if exist "%PROJECT_DIR%\static\agent\asset-agent.exe" (
    echo [%date% %time:~0,8%] Asset Agent二进制已存在，跳过构建
) else (
    echo [%date% %time:~0,8%] 正在构建Asset Agent...
    python "%PROJECT_DIR%\Auto_Setup\build_agent.py" 2>nul
    if %errorlevel% neq 0 (
        echo [%date% %time:~0,8%] Agent构建失败，但继续打包（可能已有预编译二进制）
        echo [%date% %time:~0,8%] 如需Agent功能，请确保 static\agent\asset-agent.exe 存在
    ) else (
        echo [%date% %time:~0,8%] Asset Agent构建成功
    )
)
...
:: ����Ŀ��Ŀ¼����������ڣ�
echo.
echo [%date% %time:~0,8%] ׼���ƶ��ļ�...
if not exist "%DEST_DIR%" (
    mkdir "%DEST_DIR%"
    echo [%date% %time:~0,8%] ����Ŀ��Ŀ¼: %DEST_DIR%
)

:: �ƶ�dist�ļ������ݵ�Auto_Setup/Output
echo [%date% %time:~0,8%] �����ƶ�dist�ļ������ݵ�%DEST_DIR%...
xcopy /E /H /C /R /Y "%PROJECT_DIR%\dist\*" "%DEST_DIR%\*"
if %errorlevel% gtr 1 (
    echo [%date% %time:~0,8%] �����ļ��ƶ�ʧ��
    pause
    exit /b 1
)
